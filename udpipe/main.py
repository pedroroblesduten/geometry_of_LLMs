...
if pasta_arquivos:
    ...
    with open(os.path.join(RESULTADOS_METRICAS_FOLDER, "udpipe_mean_distance_type.csv"), mode="w", newline="") as mdt_output_csv:
        csv_writer = csv.writer(mdt_output_csv)
        headers = ["File"] + [f"{relacao} (Proporção, Distância Média, Contagem)" for relacao in all_relations]
        csv_writer.writerow(headers)
        ...
        for arquivo_json, (estatisticas, contagem) in data_by_file.items():
            ...
            for relacao in all_relations:
                ...
                if proporcao is not None:
                    row_data.append(f"{proporcao}, {distancia_media}, {contagem_relacao}")
                else:
                    row_data.append("")
